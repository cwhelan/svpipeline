...
\title[Cloudbreak]{{\em Cloudbreak}: Accurate and Scalable Genomic Structural Variation Detection in the Cloud with MapReduce}
...
\address{$^{1}$Institute on Development and Disability and Center for Spoken Language Understanding, Oregon Health \& Science University, Portland, OR, USA \\
$^{2}$ Program in Molecular and Cellular Biosciences, Oregon Health \& Science University, Portland, OR, USA \\
$^{3}$ Department of Biology, University of Bari ``Aldo Moro'', Via G. Amendola 165/A, 70126, Bari, Italy \\
$^{4}$ Behavioral Neuroscience Department, Oregon Health \& Science University, Portland, OR, USA \\
...
Source code, binary releases, example scripts, and a full user manual are available at \url{http://github.com/cwhelan/cloudbreak}
...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{../../figures/workflow_with_whirr.png}
...
We have implemented an RP-based SV detection package, Cloudbreak, in the framework described above. Cloudbreak detects deletions of size 40bp to 25kb, and small insertions. We accomplish this by providing the following implementions of the user-defined functions described above:
...
It should be noted that the methods tested here vary in their breakpoint resolution (Supplementary Fig. \ref{Sbreakpoint_resolution}). SR methods have higher resolution than RP methods. Cloudbreak's resolution suffers because the independent calculation of features at each window from the distribution of overlapping insert sizes does not allow the algorithm to keep track of the actual coordinates of the mapped reads from each pair, which other tools use to set bounds on the true breakpoint locations. We believe, however, that it is worth increasing sensitivity and specificity in SV detection tools even at the cost of breakpoint resolution. \todo{It would also be possible to extend our feature set to include information useful for increasing resolution, such as the location of nearby read mappings or soft-clipped alignments. }In addition, several recent pipelines have validated RP calls \emph{in silico} by local assembly, a process that could potentially be parallelized in a subsequent MapReduce job.
